...
template <index_t BlockSize,
          ...
          AddressSpace DstAddressSpace          = AddressSpace::generic,
          InMemoryDataOperation DstInMemOp      = InMemoryDataOperation::none>
struct BlockwiseGenericTensorSliceCopy_v4_src_runtime_offsets
{
    static constexpr index_t nDim = BlockSrcDesc::GetNumOfDimension();
    using Index                   = MultiIndex<nDim>;

    __device__ constexpr BlockwiseGenericTensorSliceCopy_v4_src_runtime_offsets(
        ...
        const Index& src_runtime_strides_mul)
    {
        ...
        mThreadwiseLoad.SetSrcRuntimeStrides(src_runtime_strides_div, src_runtime_strides_mul);
        mThreadwiseLoad.SetSrcSliceOrigin(src_block_slice_origin + thread_data_id_begin);
        mThreadwiseLoad.SetDstSliceOrigin(make_zero_array<index_t, nDim>());
    ...
    using ThreadwiseLoad =
        ThreadwiseGenericTensorSliceCopy_v4r2_src_runtime_offsets<BlockSrcDesc,
                                                                  ThreadBufferDesc,
                                                                  ThreadSliceLengths,
                                                                  SrcDimAccessOrder,
                                                                  SrcVectoReadDim,
