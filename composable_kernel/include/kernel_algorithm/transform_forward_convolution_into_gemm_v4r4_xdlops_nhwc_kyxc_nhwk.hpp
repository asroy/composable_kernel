#ifndef CK_TRANSFORM_FORWARD_CONVOLUTION_INTO_GEMM_V4R4_XDLOPS_NHWC_KYXC_NHWK_HPP
#define CK_TRANSFORM_FORWARD_CONVOLUTION_INTO_GEMM_V4R4_XDLOPS_NHWC_KYXC_NHWK_HPP
...
transform_forward_convolution_into_gemm_v4r4_xdlops_nhwc_kyxc_nhwk_pad(
...
template <typename... Wei,
          ...
          typename InRightPads>
__host__ __device__ constexpr auto
transform_forward_convolution_into_gemm_v4r4_xdlops_nhwc_kyxc_nhwk_no_pad(
    const DynamicTensorDescriptor<Wei...>& wei_k_y_x_c_global_desc,
    const DynamicTensorDescriptor<In...>& in_n_hi_wi_c_global_desc,
    const DynamicTensorDescriptor<Out...>& out_n_ho_wo_k_global_desc,
    const ConvStrides& conv_strides,
    ...
    const InRightPads& in_right_pads)
{
    ...
    const auto N = in_n_hi_wi_c_global_desc.GetLength(I0);
    const auto C = in_n_hi_wi_c_global_desc.GetLength(I1);
    const auto K = out_n_ho_wo_k_global_desc.GetLength(I1);

    const auto Hi = in_n_hi_wi_c_global_desc.GetLength(I2);
    const auto Wi = in_n_hi_wi_c_global_desc.GetLength(I3);

    const auto Ho = out_n_ho_wo_k_global_desc.GetLength(I2);
    const auto Wo = out_n_ho_wo_k_global_desc.GetLength(I3);

    const auto Y = wei_k_y_x_c_global_desc.GetLength(I2);
    const auto X = wei_k_y_x_c_global_desc.GetLength(I3);
...
template <typename... Wei,
          ...
          typename InLeftPads,
          typename InRightPads>
__host__ __device__ constexpr auto transform_forward_convolution_into_gemm_v4r4_xdlops_nhwc_kyxc_nhwk_1x1(
    const DynamicTensorDescriptor<Wei...>& wei_k_y_x_c_global_desc,
    const DynamicTensorDescriptor<In...>& in_n_hi_wi_c_global_desc,
    const DynamicTensorDescriptor<Out...>& out_n_ho_wo_k_global_desc,
    const ConvStrides& conv_strides,
