#ifndef CK_GRIDWISE_CONVOLUTION_IMPLICIT_GEMM_V4R1_NCHW_KCYX_NKHW_LDS_DOUBLE_BUFFER_GENERIC_NKC_HPP
#define CK_GRIDWISE_CONVOLUTION_IMPLICIT_GEMM_V4R1_NCHW_KCYX_NKHW_LDS_DOUBLE_BUFFER_GENERIC_NKC_HPP
...
template <index_t BlockSize,
          ...
          typename TensorLengths_Hi_Wi_Y_X_Ho_Wo,
...
struct GridwiseConvolutionImplicitGemm_v4r1_nchw_kcyx_nkhw_lds_double_buffer_generic_nkc
{
                        ...
                        index_t C) const
    {
        ...
        constexpr index_t Hi = TensorLengths_Hi_Wi_Y_X_Ho_Wo{}[0];
        constexpr index_t Wi = TensorLengths_Hi_Wi_Y_X_Ho_Wo{}[1];
        constexpr index_t Y  = TensorLengths_Hi_Wi_Y_X_Ho_Wo{}[2];
        constexpr index_t X  = TensorLengths_Hi_Wi_Y_X_Ho_Wo{}[3];
        constexpr index_t Ho = TensorLengths_Hi_Wi_Y_X_Ho_Wo{}[4];
        constexpr index_t Wo = TensorLengths_Hi_Wi_Y_X_Ho_Wo{}[5];
        ...
        // TODO: move N assert to outside
        // static_assert(N % (N1 * N2) == 0, "wrong! cannot divice N evenly among thread");

        const index_t N0 = N / (N1 * N2);

        const index_t B = N0 * Ho * Wo;

        const index_t E = C * Y * X;
        ...
        // TODO: move B & E assert to outside kernel

        // const index_t KBlockWork = K / KPerBlock;
        const index_t BBlockWork = B / BPerBlock;

        const index_t BBlockID = get_block_1d_id() % BBlockWork;
        const index_t KBlockID = get_block_1d_id() / BBlockWork;

        const index_t k_block_data_on_global = KBlockID * KPerBlock;
        const index_t b_block_data_on_global = BBlockID * BPerBlock;

        // input tensor
        //     global tensor in global memory
        constexpr auto in_dn_dc_hi_wi_global_desc =
            make_native_tensor_descriptor_packed(Sequence<N1 * N2, 1, Hi, Wi>{});

        constexpr auto in_dn_dc_hip_wip_global_desc =
            transform_tensor_descriptor(in_dn_dc_hi_wi_global_desc,
                                        make_tuple(PassThrough<N1 * N2>{},
                                                   PassThrough<1>{},
                                                   Pad<Sequence<Hi, Wi>, LeftPads, RightPads>{}),
                                        make_tuple(Sequence<0>{}, Sequence<1>{}, Sequence<2, 3>{}),
                                        make_tuple(Sequence<0>{}, Sequence<1>{}, Sequence<2, 3>{}));

        constexpr index_t Hip = in_dn_dc_hip_wip_global_desc.GetLengths()[2];
        constexpr index_t Wip = in_dn_dc_hip_wip_global_desc.GetLengths()[3];

        constexpr auto in_dn0_n1_n2_dc_y_ho_x_wo_global_desc = transform_tensor_descriptor(
            in_dn_dc_hip_wip_global_desc,
            make_tuple(UnMerge<Sequence<1, N1, N2>>{},
                       PassThrough<1>{},
        ...
        constexpr auto in_de_n1_db_n2_global_desc = transform_tensor_descriptor(
            in_dn0_n1_n2_dc_y_ho_x_wo_global_desc,
            make_tuple(Merge<Sequence<1, Y, X>>{},
                       PassThrough<N1>{},
                       Merge<Sequence<1, Ho, Wo>>{},
                       PassThrough<N2>{}),
        ...
        auto blockwise_in_copy = BlockwiseGenericTensorSliceCopy_v4_src_runtime_offsets<
            BlockSize,
            decltype(in_de_n1_db_n2_global_desc),
            ...
            {0, 0, 0, 0},
            {Y * X, 1, Ho * Wo, 1},
            {Hi * Wi, N2 * C * Hi * Wi, N1 * N2 * C * Hi * Wi, C * Hi * Wi});

        // weight tensor
        //     global tensor in global memory, src of blockwise copy
        //     It is constructed differently, depending on whether forward or backward weight
        //       convolution
        constexpr auto wei_dk_dc_y_x_global_desc =
            make_native_tensor_descriptor_packed(Sequence<1, 1, Y, X>{});

        constexpr auto wei_de_dk_global_desc =
            make_wei_e_k_global_desc_v4r1_generic_nkc<ConvDirection>{}(wei_dk_dc_y_x_global_desc);
        ...
        auto blockwise_wei_copy = BlockwiseGenericTensorSliceCopy_v4_src_runtime_offsets<
            BlockSize,
            decltype(wei_de_dk_global_desc),
...
#pragma unroll
        ...
        // LDS double buffer: tail
        {
            ...
            bool has_two_iteration_left = (E % (2 * EPerBlock) == 0);
        ...
        // copy output: register to global memory
        {
            ...
            constexpr auto out_dn_dk_ho_wo_global_desc =
                make_native_tensor_descriptor_packed(Sequence<N1 * N2, K1, Ho, Wo>{});

            constexpr auto out_dn0_n1_n2_dk0_k1_ho_wo_global_desc = transform_tensor_descriptor(
                out_dn_dk_ho_wo_global_desc,
                make_tuple(UnMerge<Sequence<1, N1, N2>>{},
                           UnMerge<Sequence<1, K1>>{},
                           PassThrough<Ho>{},
                           PassThrough<Wo>{}),
                make_tuple(Sequence<0>{}, Sequence<1>{}, Sequence<2>{}, Sequence<3>{}),
            ...
            constexpr auto out_dk0_k1_n1_db_n2_global_desc = transform_tensor_descriptor(
                out_dn0_n1_n2_dk0_k1_ho_wo_global_desc,
                make_tuple(PassThrough<1>{},
                           PassThrough<K1>{},
                           PassThrough<N1>{},
                           Merge<Sequence<1, Ho, Wo>>{},
                           PassThrough<N2>{}),
            ...
            ThreadwiseGenericTensorSliceCopy_v4r2_dst_runtime_offsets<
                decltype(out_k0_k1_n1_b_n2_thread_desc),
                decltype(out_dk0_k1_n1_db_n2_global_desc),
                ...
                {k_thread_data_on_global / K1,
                 ...
                 0},
                {1, 1, 1, Ho * Wo, 1},
                {K1 * Ho * Wo, Ho * Wo, N2 * K * Ho * Wo, N1 * N2 * K * Ho * Wo, K * Ho * Wo})
